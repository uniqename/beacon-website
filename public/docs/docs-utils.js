/* ─── Beacon of New Beginnings — Document Utilities ───────────────────────────────────────
...
const DOXA_ACCENT   = '#ea580c';
const SIG_IMAGE_URL = '../images/enam-signature.png';
    ...
    <div style="background:#fff;border-radius:16px;width:480px;max-width:95vw;box-shadow:0 8px 40px rgba(0,0,0,.2);overflow:hidden;font-family:'Helvetica Neue',Arial,sans-serif;">
      ...
      <div style="display:flex;border-bottom:2px solid #e7e5e4;">
        <button id="tab-saved" onclick="switchTab('saved')"
          style="flex:1;padding:12px;border:none;background:#fff3ed;font-weight:700;font-size:12px;cursor:pointer;color:${DOXA_ACCENT};">
          Saved
        </button>
      ...
      <div id="panel-saved" style="padding:24px;text-align:center;">
        <p style="font-size:12px;color:#78716c;margin-bottom:12px;">Your saved signature — select a size then click Insert. Drag the orange handle to resize after placing.</p>
        ...
        <div style="display:flex;gap:8px;justify-content:center;margin-bottom:16px;">
          <button onclick="setSigSize('small')"  id="sz-small"  class="sz-btn" style="padding:6px 14px;border:2px solid ${DOXA_ACCENT};border-radius:20px;font-size:12px;font-weight:700;cursor:pointer;background:#fff3ed;color:${DOXA_ACCENT};">Small</button>
      ...
      <div id="panel-upload" style="display:none;padding:24px;text-align:center;">
        ...
        <div style="display:flex;gap:8px;justify-content:center;margin-bottom:8px;">
          <button onclick="setSigSize('small')"  id="sz-small"  style="padding:6px 14px;border:2px solid ${DOXA_ACCENT};border-radius:20px;font-size:12px;font-weight:700;cursor:pointer;background:#fff3ed;color:${DOXA_ACCENT};">Small</button>
          <button onclick="setSigSize('medium')" id="sz-medium" style="padding:6px 14px;border:2px solid #d6d3d1;border-radius:20px;font-size:12px;font-weight:700;cursor:pointer;background:#fff;color:#78716c;">Medium</button>
          <button onclick="setSigSize('large')"  id="sz-large"  style="padding:6px 14px;border:2px solid #d6d3d1;border-radius:20px;font-size:12px;font-weight:700;cursor:pointer;background:#fff;color:#78716c;">Large</button>
        </div>
...
function openSignatureModal() {
  ...
  var localSig  = localStorage.getItem('bnb_saved_signature');
...
function switchTab(tab) {
  ['saved', 'draw', 'upload'].forEach(function(t) {
    ...
    if (btn) {
      btn.style.background = active ? '#fff3ed' : '#fafaf9';
...
function setSigSize(size) {
  ...
  ['small','medium','large'].forEach(function(s) {
    ...
    btn.style.background  = active ? '#fff3ed'   : '#fff';
    btn.style.color       = active ? DOXA_ACCENT : '#78716c';
  });
...
/* ── Insert signature at cursor — resizable wrapper ─────────────────────── */
function insertSigAtCursor(src, h) {
  var id = 'sig-' + Date.now();
  var wrapHtml =
    '<span id="' + id + '" contenteditable="false" class="sig-node" ' +
    'style="display:inline-block;position:relative;vertical-align:middle;user-select:none;">' +
      '<img src="' + src + '" style="height:' + h + 'px;display:block;pointer-events:none;" />' +
      '<span onclick="this.parentNode.remove()" ' +
        'style="position:absolute;top:-8px;right:-8px;width:18px;height:18px;background:#ef4444;color:#fff;' +
        'border-radius:50%;font-size:12px;line-height:18px;text-align:center;cursor:pointer;z-index:10;font-weight:bold;">×</span>' +
      '<span class="sig-resize" ' +
        'style="position:absolute;bottom:-6px;right:-6px;width:14px;height:14px;background:' + DOXA_ACCENT + ';' +
        'border-radius:50%;cursor:se-resize;z-index:10;"></span>' +
    '</span>';

  insertAtCursor(wrapHtml);
  ...
  setTimeout(function() {
    ...
    if (!node) return;
    var handle = node.querySelector('.sig-resize');
...
function insertSavedSig() {
  var src = localStorage.getItem('bnb_saved_signature');
...
function saveDrawnAsDefault() {
  ...
  if (!canvas) return;
  var dataUrl = canvas.toDataURL('image/png');
  localStorage.setItem('bnb_saved_signature', dataUrl);
  var btn = document.getElementById('save-drawn-btn');
...
function saveUploadedAsDefault() {
  ...
  localStorage.setItem('bnb_saved_signature', _uploadedSrc);
...
function insertUploadedSig() {
  if (!_uploadedSrc) return;
  var h = SIG_SIZES[_sigSize];
  insertSigAtCursor(_uploadedSrc, h);
}
